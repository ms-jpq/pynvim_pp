...
from .types import Ext, NoneType, NvimPos
...
class Window(Ext):
    ...
    async def list(cls) -> Sequence[Window]:
        return cast(Sequence[Window], await cls.api.list_wins(NoneType))

    @classmethod
    async def get_current(cls) -> Window:
        return await cls.api.get_current_win(Window)

    @classmethod
    async def set_current(cls, win: Window) -> None:
        await cls.api.set_current_win(NoneType, win)
